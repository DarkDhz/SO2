...
void *th_consumer(void *arg) {
    ...
    // create a temporally cell to avoid producer write in the same cell
    // we are reading
    ...
    while (!state) {
      // Accessing shared data, must lock
      pthread_mutex_lock(&data);

      // if no data available, wait the signal from producer
      while ((buffer->counter == 0) && (!state)) {
        pthread_cond_wait(&consumer, &data);
      }

      // check if is finish state
      if ((buffer->counter == 0) && (state)) {
        break;
      } else {
      	// read the cell from the buffer
        bufferCell = buffer->cells[buffer->read];
        // change the buffer cell for our temporally cell
        buffer->cells[buffer->read] = toRead;
        // assign the cell we want to read
        toRead = bufferCell;
        ...
        // decrease the number of cells counter
        buffer->counter--;
      ...
      for (i = 0; i < toRead->nelems; i++) {
        invalid = extract_fields_airport(origin, destination, toRead->lines[i]);
        if (!invalid) {
          ...
          if ((index_origin >= 0) && (index_destination >= 0)){
          	// editing shared matrix, must lock
...
void read_airports_data(int **num_flights, char **airports, char *fname) {
    ...
    cell *toAdd;
    int i,j;
    // Reserve all cells in the buffer
    ...
    // Parameters for consumer threads
    ...
    // read the file header and discard it
    fgets(line, MAXCHAR, fp);

    // generate a temporally cell to insert into the buffer
    ...
    while (1) {
      ...
      if (producerCell->nelems > 0) {
        ...
        // increase the number of cells with data in the buffer
        buffer.counter++;
        // say the consumer that can read
        pthread_cond_signal(&consumer);
        pthread_mutex_unlock(&data);
        // reset the temporal cell
        producerCell->nelems = 0;
    ...
    // change state to 1, it will say to consumers that no more data will be inserted
    pthread_mutex_lock(&data);
    state = 1;
    pthread_cond_signal(&consumer);
    pthread_mutex_unlock(&data);
    ...
    // free all allocated memory in the buffer
    cell *toRemove;
