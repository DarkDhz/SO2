...
// Definim els nostres mutex
pthread_mutex_t extract = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t save = PTHREAD_MUTEX_INITIALIZER;

// Definim l'estructura de parametres que pasarem a thr_fn
struct param {
  FILE *fp;
  int N;
  int **num_flights;
...
void *thr_fn(void *arg) {
  ...
  int i;
  // Variable que guarde l'informació lleguida del fitxer
  data = (char **) malloc(sizeof(char*) * info->N);

  for (i = 0; i < info->N; i++) {
    data[i] = malloc(sizeof(char) * MAXCHAR);
  }
  ...
  do {
    ...
    it = 0;
    // Fem un lock per evitar que els diferents threads llegeixin a la vegada
    pthread_mutex_lock(&extract);
    while ((it < info->N) && (fgets(data[it], MAXCHAR, info->fp) != NULL)) {
      it++;
    }
    pthread_mutex_unlock(&extract);
  
    // Extraiem la informació de les linies llegides
    for (i = 0; i < info->N; i++) {
      invalid = extract_fields_airport(origin, destination, data[i]);
      ...
      if (!invalid) {
        ...
        if ((index_origin >= 0) && (index_destination >= 0)) {
          // Fem un lock per evitar que els diferetns threads
          // modifiquin a la vegada la matriu de vols
...
{
  char line[MAXCHAR];
  // Definim N = nombre de linies procesades a la vegada
  int N = 100;
  FILE *fp;
  ...
  // Parametres per la funcio thr_fn()
  struct param info = {fp, N, num_flights, airports};
  
  // Creem els diferents thread
  err = pthread_create(&th[0], NULL, thr_fn, (void*) &info);
  if (err != 0) {
    printf("error al crear el fil\n");
    exit(1);
  }
  
  err = pthread_create(&th[1], NULL, thr_fn, (void*) &info);
  if (err != 0) {
    printf("error al crear el fil\n");
    exit(1);
  }
